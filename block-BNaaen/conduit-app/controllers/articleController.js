...
let feedArticles = async (req, res, next) => {
  var limit = 20;
  var skip = 0;
  if (req.query.limit) {
...
let listArticle = async (req, res, next) => {
...
let createArticle = async (req, res, next) => {
...
let getCommentForArticle = async (req, res, next) => {
  let slug = req.params.slug;
  let id = req.user ? req.user.userId : false;
  try {
    let article = await Article.findOne({ slug });
...
let makeFavouriteArticle = async (req, res, next) => {
  let slug = req.params.slug;
  try {
    let article = await Article.findOne({ slug });
    if (!article) {
...
let unFavoriteArticle = async (req, res, next) => {
  let slug = req.params.slug;
  try {
    let article = await Article.findOne({ slug });
    if (!article) {
